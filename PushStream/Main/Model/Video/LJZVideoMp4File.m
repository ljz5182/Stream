...
//已经完成的文件列表
@property (nonatomic, strong) LJZVideoInFoList *completedFileList;

@property (nonatomic, strong) LJZVideoFlvStream *flvStream;
...
@implementation LJZVideoMp4File
...
-(LJZVideoInFoList *)completedFileList{
    if (!_completedFileList) {
        _completedFileList = [LJZVideoInFoList new];
    }
    return _completedFileList;
}

-(LJZVideoFlvStream *)flvStream{
    if (!_flvStream) {
        _flvStream = [LJZVideoFlvStream new];
    }
    return _flvStream;
}
